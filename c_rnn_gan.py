...
class Generator(nn.Module):
    ...
    def __init__(self, num_feats, hidden_units=512, drop_prob=0.5, use_cuda=False):
        ...
        self.fc_condition = block(self.args.seq_len, self.args.seq_len)
        self.fc_input = block(self.args.seq_len, self.args.seq_len)
        self.fc_syn = block(self.args.seq_len*2, self.args.seq_len*2)


        self.lstm1 = nn.LSTM(input_size=self.num_feats, hidden_size=hidden_units,
                            num_layers=2, batch_first=True, dropout=drop_prob)
        self.lstm2 = nn.LSTM(input_size=self.num_feats, hidden_size=hidden_units,
                             num_layers=2, batch_first=True, dropout=drop_prob)

        self.fc_layer = nn.Sequential(
            nn.Linear(in_features=hidden_units*2, out_features=hidden_units),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Linear(in_features=hidden_units, out_features=num_feats),
            nn.LeakyReLU(0.2, inplace=True)
            )
        self.fc1 = block(self.args.seq_len, self.args.seq_len*2)
        self.fc2 = block(self.args.seq_len*2, self.args.seq_len)
    ...
    def forward(self, z, states, labels):
        ''' Forward prop
        '''
        real_batch_size = z.shape[0]
        if self.use_cuda:
            z = z.cuda()
        z = z.view([real_batch_size, self.args.seq_len, -1])
        labels = labels.view([real_batch_size, self.args.seq_len, -1])
        # # padding = torch.zeros([16, 1]).cuda()
        # # labels = torch.cat([padding, labels.view(-1, 1)], dim=1)
        # a = self.fc_input(z)
        # b = self.fc_condition(labels)
        # # a = self.fc1(a).view(note_seq.shape[0], note_seq.shape[1], -1)
        # z1 = torch.cat([a, b], dim=1)
        # z2 = self.fc_syn(z1)
        # z = z2.view(z.shape[0], self.args.seq_len, -1)


        # z: (batch_size, seq_len, num_feats)
        # z here is the uniformly random vector
        states2 = self.init_hidden(real_batch_size)
        lstm_out1, states = self.lstm1(z, states)
        lstm_out2, states2 = self.lstm2(labels, states2)
        lstm_out = torch.concat((lstm_out1, lstm_out2), 2)

        gen_feats = self.fc_layer(lstm_out).squeeze()  #128,50,2
        x = self.fc1(gen_feats.view(-1, self.args.seq_len))
...
class Discriminator(nn.Module):
    ...
    def forward(self, note_seq, state, labels):
        ...
        a = self.fc_input(note_seq)
        b = self.fc_condition(labels)
        # a = self.fc1(a).view(note_seq.shape[0], note_seq.shape[1], -1)
        z1 = torch.cat([a, b], dim=1)
        z2 = self.fc_syn(z1)
        note_seq = z2.view(note_seq.shape[0], self.args.seq_len, -1)
